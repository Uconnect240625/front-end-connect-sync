  ...
  return <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex justify-between mb-6 overflow-x-auto gap-2 pb-2">
          {days.map((day, index) => <button key={day} className={`px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${activeDay === day ? 'bg-red-600 text-white shadow-lg' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`} onClick={() => switchDay(day)}>
              {dayLabels[index]}
            </button>)}
        </div>

        <div className="space-y-4">
          {menuData[activeDay] ? getSortedMeals(menuData[activeDay]).map(([meal, items]) => <div key={meal} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              </div>) : <div className="bg-white rounded-xl p-8 shadow-sm text-center border border-gray-100">
