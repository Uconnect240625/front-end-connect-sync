...
const PGFinder = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: roommateResponse, error: roommateError } = await supabase
      ...
      setRoommateRequests(roommateResponse || []);
    } catch (error) {
        ...
        <Tabs defaultValue="pg-listings" className="w-full">
          ...
          <TabsContent value="roommate-requests" className="mt-6">
            ...
            {roommateRequests.length === 0 ? (
              <div className="text-center py-12 bg-card rounded-xl border border-border">
                <div className="text-6xl mb-4">🧑‍🤝‍🧑</div>
                <h3 className="text-xl font-semibold text-card-foreground mb-2">No Roommate Requests Available</h3>
                ...
                {roommateRequests.map(request => (
                  <div key={request.id} className="bg-card rounded-xl shadow-lg p-6 border border-border hover:shadow-xl transition-shadow">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <p><strong className="text-card-foreground">Preferred Location:</strong> <span className="text-muted-foreground">{request.location}</span></p>
                      {request.preferences && (
